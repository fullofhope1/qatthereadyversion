...
<style>
    ...
    .btn-provider { background: linear-gradient(135deg, #dc3545, #c82333); } /* Red for Day 2 */
...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-10">
            <h2 class="text-center mb-4 mt-3 fw-bold text-dark"><i class="fas fa-history text-danger me-2"></i> بيع بقايا ثاني</h2>
    ...
    <form action="requests/process_sale.php" method="POST" id="saleForm">
        ...
        <input type="hidden" name="qat_status" id="i_status" value="Leftover2">
        <input type="hidden" name="source_page" value="leftovers_2">
        ...
        <div id="step1" class="step-container active">
            ...
            <div class="grid-container">
                <?php foreach ($types as $t): ?>
                    <?php 
                        // Only show types that exist in $leftoverStocks with status Momsi_Day_2
                        $hasStock = false;
                        foreach ($leftoverStocks as $s) {
                            if ($s['qat_type_id'] == $t['id'] && $s['status'] === 'Momsi_Day_2') {
                                $hasStock = true;
                                break;
                            }
                        }
        ...
        <!-- STEP 2: Provider (Providers from leftovers Day 2) -->
        <div id="step2" class="step-container">
            <h3>اختر الدفعة (بقايا ثاني)</h3>
        ...
        <div id="step5" class="step-container">
            <h3>السعر</h3>
            <div class="grid-container">
                <button type="button" class="circle-btn btn-price" onclick="nextStep(5, 20000)">20,000</button>
...
<script>
    ...
    function populateProviders(typeId) {
        ...
        const providers = allStocks.filter(s => {
            return s.qat_type_id == typeId && s.status === 'Momsi_Day_2';
        });
        if (providers.length === 0) {
            grid.innerHTML = `<div class="alert alert-danger w-100">لا توجد بقايا (يوم ثاني) لهذا النوع حالياً</div>`;
        } else {
            providers.forEach(p => {
    ...
    function manualTrash() {
        ...
        const reason = prompt("سبب الإتلاف (اختياري):", "تالفة / غير صالحة (بقايا ثاني)");
        const formData = new FormData();
        formData.append('leftover_id', lid);
        formData.append('reason', reason || "Manual Trash Day 2");
