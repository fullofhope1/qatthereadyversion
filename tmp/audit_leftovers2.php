...
$out .= "STOCKS RETURNED BY SERVICE: " . count($stocks) . "\n";
foreach ($stocks as $s) {
    $out .= "ID: {$s['id']} | Type: {$s['qat_type_id']} | Provider: {$s['provider_name']} | Status: {$s['status']} | RemKg: {$s['remaining_kg']} | SaleDate: {$s['sale_date']} | SourceDate: {$s['source_date']}\n";
}
...
foreach ($raw as $r) {
    $out .= "ID: {$r['id']} | Status: {$r['status']} | Type: {$r['qat_type_id']} | Source: {$r['source_date']} | SaleDate: {$r['sale_date']} | Weight: {$r['weight_kg']}\n";
}

file_put_contents('C:/xampp/htdocs/qat/tmp/audit_out3.txt', $out);
echo "Written to C:/xampp/htdocs/qat/tmp/audit_out3.txt";
